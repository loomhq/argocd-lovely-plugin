...
- Update the Dockerfile.legacy to use the `<tag>` from above as the source
- Build/push the legacy image
- Test to make sure it works by updating the image specified for the argocd-repo-server init container
- Then re-tag the image as latest once functionality is validated
